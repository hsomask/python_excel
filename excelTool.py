...
def create_summary_sheet(new_wb, deposit_records, extract_records):
    """将所有人的记录写入同一个sheet"""
    summary_ws = new_wb.create_sheet(title="汇总")

    # --- 生成固定样式 start ---
    summary_ws.merge_cells('A1:K1')
    summary_ws['A1'].value = "附件1"
    summary_ws['A1'].font = Font(name='宋体', size=20, bold=True)
    summary_ws['A1'].alignment = Alignment(horizontal='left', vertical='center')

    summary_ws.merge_cells('A2:K2')
    summary_ws['A2'].value = "住房公积金基本信息"
    summary_ws['A2'].font = Font(name='宋体', size=20, bold=True)
    ...
    for cell, value in headers_L1.items():
        summary_ws[cell].value = value
        summary_ws[cell].font = Font(name='宋体', size=11, bold=True)
        summary_ws[cell].alignment = Alignment(horizontal='center', vertical='center')

    # 合并L1标题单元格
    summary_ws.merge_cells('A3:A4')
    summary_ws.merge_cells('B3:B4')
    summary_ws.merge_cells('C3:C4')
    summary_ws.merge_cells('D3:D4')
    summary_ws.merge_cells('E3:H3')
    summary_ws.merge_cells('I3:K3')
    ...
    for sheet_name, person_data in deposit_records.items():
        ...
        jc_records = person_data['jc_records']
        # 按照每个缴存记录展开
        for jc in jc_records:
            # 找到对应的提取记录
            matching_extracts = [e for e in person_extract_records if e.get('deposit_range') == jc['date_range']]
            if matching_extracts:
                for ext in matching_extracts:
                    summary_ws.cell(row=row_idx, column=1, value=seq)
                    summary_ws.cell(row=row_idx, column=2, value=person_data['name'])
                    summary_ws.cell(row=row_idx, column=3, value=person_data['id_num'])
                    summary_ws.cell(row=row_idx, column=4, value=person_data['grzhye'])
                    summary_ws.cell(row=row_idx, column=5, value=jc['date_range'])
                    summary_ws.cell(row=row_idx, column=6, value=jc['company'])
                    summary_ws.cell(row=row_idx, column=7, value=jc.get('base', ''))
                    summary_ws.cell(row=row_idx, column=8, value=jc.get('amount', ''))
                    summary_ws.cell(row=row_idx, column=9, value=ext.get('date_range', ''))
                    summary_ws.cell(row=row_idx, column=10, value=ext.get('amount', ''))
                    summary_ws.cell(row=row_idx, column=11, value=ext.get('type', ''))
                    row_idx += 1
            else:
                # 没有提取记录也要写一行
                summary_ws.cell(row=row_idx, column=1, value=seq)
                ...
                summary_ws.cell(row=row_idx, column=7, value=jc.get('base', ''))
                summary_ws.cell(row=row_idx, column=8, value=jc.get('amount', ''))
                summary_ws.cell(row=row_idx, column=9, value='')
                summary_ws.cell(row=row_idx, column=10, value='')
                summary_ws.cell(row=row_idx, column=11, value='')
                row_idx += 1
        seq += 1
    ...
    for row in summary_ws.iter_rows(min_row=1, max_row=summary_ws.max_row, min_col=1, max_col=11):
        for cell in row:
            # 跳过合并的单元格
            if not isinstance(cell, MergedCell):
                cell.border = thin_border
            # 手动为合并单元格区域的主单元格添加边框
            for merged_range in summary_ws.merged_cells.ranges:
                if cell.coordinate in merged_range:
                    ...
                    for row_num in range(merged_range.min_row, merged_range.max_row + 1):
                        for col_num in range(merged_range.min_col, merged_range.max_col + 1):
                            summary_ws.cell(row=row_num, column=col_num).border = thin_border
                    break

    # 合并姓名、证件号码、账户余额三列的相同内容
    if row_idx > 5:
        start = 5
        while start < row_idx:
            # 找到连续相同的区间
            end = start
            while end + 1 < row_idx and \
                summary_ws.cell(row=end + 1, column=2).value == summary_ws.cell(row=start, column=2).value:
                end += 1
            
            if end > start:
                # 合并姓名、证件号码、账户余额
                for col_num in [2, 3, 4]:
                    summary_ws.merge_cells(start_row=start, end_row=end, start_column=col_num, end_column=col_num)
                    cell = summary_ws.cell(row=start, column=col_num)
                    cell.alignment = Alignment(horizontal='center', vertical='center')
            start = end + 1
    
    # 添加查询截止时间行
    last_row = summary_ws.max_row + 1
    summary_ws.merge_cells(f'A{last_row}:K{last_row}')
    summary_ws[f'A{last_row}'].value = "查询截止时间为YYYY年MM月DD日HH时MI分"
    summary_ws[f'A{last_row}'].font = Font(name='宋体', size=12)
    summary_ws[f'A{last_row}'].alignment = Alignment(horizontal='center', vertical='center')
...
def main():
        ...
        try:
            with pd.ExcelFile(extract_file_path) as excel:
                ...
                if extract_only_sheets:
                    logger.info(f"以下sheet只在提取文件中存在的sheet: {extract_only_sheets}")
        except Exception as e:
    ...
    # 只创建一个汇总sheet
    create_summary_sheet(new_wb, deposit_records, extract_records)
